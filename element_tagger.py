...
One physical event is ONE element. A grease fire is a single SPECIAL_EFFECTS \
entry — do not also tag the smoke it makes, the flames, and the scorch it \
leaves. Tag a second effect only when a different department supplies it \
separately: the fire and the extinguisher discharge that puts it out are two \
things; the fire and its flames are one. Over-tagging inflates the budget.

MAKEUP_HAIR names the condition, never the person. "EXTINGUISHER POWDER IN \
HAIR", "FOREARM SCARS", "WIND-BURNED FACE" — never a bare character name. If \
you cannot name a specific makeup or hair requirement, do not tag one.
